...
public class RagePixelSpriteSheetEditorWindow : EditorWindow
{
	...
	public void OnGUI()
	{
		...
		if(spriteSheet != null)
		{
			...
			spriteSheet.GetRow(spriteSheetGUI.currentRowKey).fontCharacter =
				EditorGUI.TextField(
					new Rect(x, y, Mathf.Min(350, Screen.width - x * 2), 16), "Font Character", spriteSheet.GetRow(spriteSheetGUI.currentRowKey).fontCharacter);
			y += 20;
			spriteSheet.GetRow(spriteSheetGUI.currentRowKey).fontYOffset =
				EditorGUI.IntField(
					new Rect(x, y, Mathf.Min(200, Screen.width - x * 2), 16), "Font Y Offset", (int)spriteSheet.GetRow(spriteSheetGUI.currentRowKey).fontYOffset);
			y += 20;
